...
class TestQdrantContextWriter:
    ...
    def test_add_splits_and_adds_documents(
        ...
        mock_qrant_vector_store = create_autospec(QdrantVectorStore, spec_set=True, instance=True)
        mock_splitter = create_autospec(TextSplitter, spec_set=True, instance=True)

        mock_chunks = [create_autospec(LangchainDocument, instance=True)]
...
class TestQdrantContextReader:
    ...
    def test_search_invokes_similarity_and_reranking(
        ...
        for doc in sample_context_documents:
            langchain_doc = create_autospec(LangchainDocument, instance=True)
            langchain_doc.page_content = doc.content
            langchain_doc.metadata = {'title': doc.title, 'modified_at': doc.modified_at.isoformat()}
            qdrant_docs.append(langchain_doc)

        mock_qrant_vector_store = create_autospec(QdrantVectorStore, spec_set=True, instance=True)
        mock_qrant_vector_store.similarity_search.return_value = qdrant_docs
        mock_ranker = create_autospec(CrossEncoder, spec_set=True, instance=True)
